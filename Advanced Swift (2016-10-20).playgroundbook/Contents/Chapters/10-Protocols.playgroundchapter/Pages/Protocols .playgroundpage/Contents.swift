...
are special: we can't use them as standalone types; we can only use them as type
constraints. This may sound like a small limitation, but it makes protocols with
associated types almost entirely different things in practice. We'll take a
closer look at this later. We'll also discuss type erasers (such as
`AnyIterator`) as a way of making it easier to work with protocols with
associated types.
...
Not relying on subclassing also makes types more flexible. In Swift, a class can
only have a single superclass. When we create a class, we have to choose the
superclass well, because we can only pick one; we can't subclass both
`AbstractSequence`, and, say, `Stream`. This can sometimes be a problem. There
are examples in Cocoa — e.g. with `NSMutableAttributedString`, where the
designers had to choose between `NSAttributedString` and `NSMutableString` as a
superclass.

Some languages have multiple inheritance, the most famous being C++. But this
...
classes. When you do, you risk major refactoring. And if you aren't the owner of
these subclasses, you can't do it at all\!
