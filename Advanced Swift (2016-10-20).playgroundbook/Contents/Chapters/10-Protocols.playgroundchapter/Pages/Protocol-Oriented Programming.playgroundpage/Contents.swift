...
we can render graphics in a Core Graphics `CGContext` and create an SVG. To
start, we'll define a protocol that describes the minimum functionality of our
drawing API:
...
simple extensions: the `svgAttributes` property on `CGRect` creates a dictionary
that matches the SVG specification. `String.init(hexColor:)` takes a `UIColor`
and turns it into a hexadecimal string (such as `"#010100"`). With these
...
/*:
We can now write drawing code that's independent of the rendering target; it
only assumes that `context` conforms to the `Drawing` protocol. If we
initialized it with a `CGContext` instead, we wouldn't need to change any of the
code:
...
using inheritance:
  ...
  - We can conform existing types to a protocol (e.g. we made `CGContext`
    conform to `Drawing`). Subclassing isn't as flexible; we can't retroactively
    change the superclass of `CGContext`.
