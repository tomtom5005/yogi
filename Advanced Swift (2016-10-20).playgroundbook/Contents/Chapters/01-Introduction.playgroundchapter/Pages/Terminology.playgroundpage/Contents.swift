...
only has value semantics if the elements have value semantics too. In the next
chapter, we'll look at how Swift's collections differ from Foundation
collections such as `NSArray` and `NSDictionary`.
...
expressions can be closures. Functions declared with the `func` keyword are
closures too.

Functions are held by reference. This means assigning a function that has state
via closed-over variables to another variable doesn't copy that state; it shares
it, similar to object references. What's more is that when two closures close
over the same local variable, they both share that variable, so they share
state. This can be quite surprising, and we'll discuss this more in the chapter
on functions.

Functions defined inside a class or protocol are **methods**, and they have an
implicit `self` parameter. A function that, instead of taking multiple
arguments, takes some arguments and returns another function representing the
partial application of the arguments to that function, is a **curried
function**. Sometimes we call functions that aren't methods **free functions**.
This is to distinguish them from methods.

Free functions, and methods called on structs, are **statically dispatched**.
This means the function that'll be called is known at compile time. This also
...
execute. It can also discard or simplify code that it can prove at compile time
won't actually run.
...
Java or C++ dynamic dispatch works), or in the case of `@objc` classes and
