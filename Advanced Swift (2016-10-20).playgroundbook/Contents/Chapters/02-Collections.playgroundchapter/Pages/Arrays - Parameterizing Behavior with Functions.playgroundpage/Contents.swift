...
library. There are more than a dozen separate functions that take a closure that
allows the caller to customize the key step:
...
These help make the code even more readable. Ascending order sort is natural, so
the meaning of `array.sort()` is intuitive. `array.index(of: "foo")` is clearer
...
There are other functions of similar usefulness that would also take a closure
...
define them yourself (and might like to try):
  ...
  - **`prefix(while:)`** — filter elements while a predicate returns true, then
    drop the rest (similar to `filter`, but with an early exit, and useful for
    infinite or lazily computed sequences)

  - **`drop(while:)`** — drop elements until the predicate ceases to be true,
    and then return the rest (similar to `prefix(while:)`, but this returns the
    inverse)

Many of these we define elsewhere in the book. (`prefix(while:)` and
`drop(while:)` are actually [planned additions to the standard
library](https://github.com/apple/swift-evolution/blob/master/proposals/0045-scan-takewhile-dropwhile.md),
but they didn't make the cut for Swift 3.0. They'll be added in a future
release.)
...
`last(where:)` wraps this logic — we use a closure to abstract over the part of
