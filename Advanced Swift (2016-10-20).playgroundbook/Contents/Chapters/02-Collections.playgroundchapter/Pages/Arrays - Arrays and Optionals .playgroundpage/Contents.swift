...
rare in Swift to actually need to calculate an index:
  ...
  - Want to iterate over all but the first element of an array? `for x in
    array.dropFirst()`

  - Want to iterate over all but the last 5 elements? `for x in
    array.dropLast(5)`

  - Want to number all the elements in an array? `for (num, element) in
    collection.enumerated()`

  - Want to find the location of a specific element? `if let idx = array.index {
    someMatchingLogic($0) }`

  - Want to transform all the elements in an array? `array.map {
...
Other operations behave differently. The `first` and `last` properties return an
optional; they return `nil` if the array is empty. `first` is equivalent to
...
last element if the array isn't empty and otherwise do nothing and return `nil`.
Which one you'd want to use depends on your use case. When you're using the
array as a stack, you'll probably always want to combine checking for `empty`
and removing the last entry. On the other hand, if you already know through
invariants whether or not the array is empty, dealing with the optional is
fiddly.
