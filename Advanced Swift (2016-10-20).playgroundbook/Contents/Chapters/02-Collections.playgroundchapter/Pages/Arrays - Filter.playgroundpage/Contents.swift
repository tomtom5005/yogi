...
arrays without having to introduce a single intermediate array, and the
...
filtered elements just to count them, and it exits early, as soon as it matches
the first element. Generally, only ever use `filter` if you want all the
results.

Often you want to do something that can be done by `contains`, but it looks
