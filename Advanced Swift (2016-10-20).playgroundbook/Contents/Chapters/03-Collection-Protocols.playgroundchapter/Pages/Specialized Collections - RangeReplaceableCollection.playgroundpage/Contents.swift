...
that comes a whole bunch of derived methods for free:
  ...
  - **`insert(at:)`** and **`insert(contentsOf:)`** — replace
    `atIndex..<atIndex` (i.e. replace the empty range at that point in the
    array) with a new element/elements
...
`MutableCollection` is `String.CharacterView`. The reasons boil down to what we
said above about indices having to remain stable during a single-element
subscript mutation, which `CharacterView` can't guarantee. We'll talk more about
