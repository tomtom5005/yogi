...
of the original sequence's elements and an incrementing number. Just like our
wrapper of `readLine`, elements are lazily generated. The consumption of the
base sequence only happens when you move through the enumerated sequence using
its iterator, and not when it's created. So if you run the above code from the
command line, you'll see it waiting inside the `for` loop. It prints the lines
you type in as you hit return; it does *not* wait until the input is terminated
with control-D. But nonetheless, each time `enumerated` serves up a line from
`standardIn`, it's consuming the standard input. You can't iterate over it twice
