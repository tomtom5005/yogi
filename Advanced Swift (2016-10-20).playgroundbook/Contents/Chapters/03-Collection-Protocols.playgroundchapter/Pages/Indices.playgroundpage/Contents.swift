...
would be. Also, subscripting on an index is expected to give direct element
...
In contrast, subscripting on an index is part of the `Collection` protocol and
...
programmer error and doing so will trap:
...
key-value pair and not just a `Value`.
