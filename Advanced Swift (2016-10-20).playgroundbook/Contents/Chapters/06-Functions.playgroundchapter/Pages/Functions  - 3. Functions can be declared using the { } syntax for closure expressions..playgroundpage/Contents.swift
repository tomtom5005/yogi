...
demonstrated above. The other way is to use a *closure expression*. Consider
this simple function to double a number:
...
keyword. The only way they can be used is if you assign them to a variable when
they're created (as we do here with `doubler`), or if you pass them to another
function or method.

The doubler declared using the closure expression, and the one declared earlier
using the `func` keyword, are completely equivalent. They even exist in the same
