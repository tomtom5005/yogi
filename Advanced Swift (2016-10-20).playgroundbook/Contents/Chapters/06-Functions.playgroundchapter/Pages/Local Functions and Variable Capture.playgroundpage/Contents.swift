...
sort](http://en.wikipedia.org/wiki/Merge_sort). The merge sort algorithm is made
up of two parts: a division into sublists of one element, followed by a merge of
those lists. Often, it's nice to define `merge` as a separate function. But this
leads to a problem — `merge` requires some temporary scratch storage:
