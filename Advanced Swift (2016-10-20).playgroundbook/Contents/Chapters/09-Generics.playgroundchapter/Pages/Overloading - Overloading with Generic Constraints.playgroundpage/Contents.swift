...
This version of `isSubset` comes with a big downside, and that's performance.
The algorithm has performance characteristics of `O(n*m)`, where *n* and *m* are
the element counts of the two arrays. That is, as the input sizes grow, the
...
loop results in an `O(n*m)` function.

This is fine for small inputs, and perhaps you're only ever going to call this
with arrays with entries in the hundreds. But call it with arrays with thousands
or millions of elements and you'll be sorry.
...
`O(n*m)` for the `Equatable` version — if both arrays have 1,000 elements, it's
...
they are in the case of `isSubset` — the more specific function is probably the
better choice.
