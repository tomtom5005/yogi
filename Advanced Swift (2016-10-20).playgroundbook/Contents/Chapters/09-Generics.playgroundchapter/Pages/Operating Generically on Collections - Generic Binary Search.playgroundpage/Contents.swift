...
to call `index(after:)` to get to it. This means it must be an integer (though
not necessarily an `Int`). So this is a constraint in the definition of
`Collection`:
...
what would that mean? If you had the linked list from the collection protocols
chapter, you obviously couldn't "add" the pointers to two nodes together.
Instead, we must think only in terms of moving indices by some distance using
`index(after:)`, `index(before:)`, or `index(_:offsetBy:)`.
