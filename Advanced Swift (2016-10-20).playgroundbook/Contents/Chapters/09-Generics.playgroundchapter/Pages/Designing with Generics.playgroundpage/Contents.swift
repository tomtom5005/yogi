...
asynchronously and then calls the callback with the list of fetched users.
...
the JSON response, which will give us arrays of dictionaries. Finally, we
...
/*:
This function has three possible error cases: the URL loading can fail, the JSON
parsing can fail, and building user objects from the JSON array can fail. All
three operations return `nil` upon failure. By using `flatMap` on the optional
values, we ensure that subsequent operations are only executed if the previous
ones succeeded. Otherwise, the `nil` value from the first failing operation will
be propagated through the chain to the end, where we eventually call the
callback either with a valid users array or `nil`.
