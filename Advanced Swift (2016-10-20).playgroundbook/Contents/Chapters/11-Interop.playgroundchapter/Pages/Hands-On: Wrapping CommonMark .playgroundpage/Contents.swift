...
we need to start is the dynamic library and its header files.
...
think that we need to convert the Swift string into an `Int8` pointer. However,
bridging between native and C strings is such a common operation that Swift will
do this automatically for us. We do have to be careful with the `len` parameter,
as the function expects the length of the UTF-8-encoded string in bytes, and not
the number of characters. We get the correct value from the string's `utf8`
...
> you can't use the automatic bridging. However, it's often still easy. For
> example, if you need an array of UTF-16 code points, you can use
> `Array(string.utf16)`.
...
We turn the function into a convenience initializer for our class. Note that the
function can return `nil` if parsing fails. Therefore, our initializer should be
failable and return `nil` (the optional value, not the null pointer) if this
occurs:
...
for every case in an enum, a top-level variable is generated. Enums marked with
the `NS_ENUM` macro, used by Apple in its Objective-C frameworks, are imported
as native Swift enumerations. Additionally, you can annotate your enum cases
with `swift_name` to make them member variables:
...
sequence. In that case, the code below will return wrong values, or even worse,
crash. Depending on your use case, returning a lazily constructed sequence might
be exactly what you want, but if your data structure can change, returning an
array is a much safer choice.
...
an array of `Block` elements that represent the list items:
...
creates `Block` and `InlineElement` values from the `Node` type, and another
pair reconstructs a `Node` from these enums. This allows us to write functions
that transform either `InlineElement` or `Block` values and reconstruct a
CommonMark document, which can then be rendered into HTML, into man pages, or
back into Markdown text.

Let's start by writing an initializer that converts a `Node` into an
`InlineElement`. We switch on the node's type and construct the corresponding
`Inline` value. For example, for a text node, we take the node's string
contents, which we access through the `literal` property in the `cmark` library.
We can safely force-unwrap `literal` because we know that text nodes always have
this value, whereas other node types might have `nil` values for `literal`. For
example, emphasis and strong nodes only have child nodes, and no literal value.
To parse the latter, we map over the node's children and call our initializer
...
It's now very easy to write simple functions that operate on our new data types.
For example, if we want to build a list of all the level one and two headers
from a Markdown document for a table of contents, we can just loop over all
children and check whether they are headers and have the correct level:
