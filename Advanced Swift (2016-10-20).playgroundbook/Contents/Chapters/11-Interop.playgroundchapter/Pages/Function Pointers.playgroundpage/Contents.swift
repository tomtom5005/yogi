...
Let's have a look at wrapping the standard C `qsort` sorting function. The type
as it's imported in Swift's Darwin module (or if you're on Linux, Glibc) is
given below:
...
Let's look at each of the arguments being passed to `qsort`:
  ...
  - Third, to get the width of each element, we use `MemoryLayout.stride`, *not*
    ...
    be the case for all types — for example, `MemoryLayout<Character>.size` is
    `9` and `MemoryLayout<Character>.stride` is `16`. When translating code from
    C to Swift, you probably want to write `MemoryLayout.stride` in cases where
    you would have written `sizeof` in C.
...
the limit of C function pointers. At the time of writing this book, the Swift
compiler segfaulted on the code below. And even if it hadn't, the code is still
impossible: it captures things from outside the closure. More specifically, it
captures the comparison and equality operators, which are different for each
generic parameter. There's nothing we can do about this — we simply encountered
a limitation of the way C works:
...
> In the case of functions that don't have any context, this address will be
> static and known at compile time. However, in case of a generic function, an
> extra parameter (the generic type) is passed in. Therefore, there are no
> addresses for specialized generic functions. This is the same for closures.
> Even if the compiler could rewrite a closure in such a way that it'd be
> possible to pass it as a function pointer, the memory management couldn't be
> done automatically — there's no way to know when to release the closure.

In practice, this is a problem for many C programmers as well. On OS X, there's
a variant of `qsort` called `qsort_b`, which takes a block — instead of a
function pointer — as the last parameter. If we replace `qsort` with `qsort_b`
in the code above, it'll compile and run fine.

However, `qsort_b` isn't available on most platforms. And other functions aside
from `qsort` might not have a block-based variant, either. Most C APIs that work
with callbacks offer a different solution. They take an extra `UnsafeRawPointer`
as a parameter and pass that pointer on to the callback function. The user of
the API can then use this to pass an arbitrary piece of data to each invocation
of the callback function. `qsort` also has a variant, `qsort_r`, which does
exactly this. Its type signature includes an extra parameter, `thunk`, which is
an `UnsafeRawPointer`. Note that this parameter has also been added to the type
of the comparison function pointer because `qsort_r` passes the value to that
