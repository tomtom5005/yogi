...
`filter`. To do that, we can add a function named `all` to `Sequence`. Like
`filter`, `all` takes a function that performs the condition check as an
argument. The difference to `filter` is the return type. `all` returns `true` if
all elements in the sequence satisfy the condition, whereas `filter` returns the
elements themselves:
