...
associated values. (We could've also used a struct or class instead; any type
that conforms to the `Error` protocol can be used as an error in a throwing
function.) For example, a file parser could choose to model the possible errors
like this:
...
> expect them to be an opt-in feature. You'll be able to specify the concrete
> error types your functions will throw, but you won't have to.
> 
> Because errors in Swift are untyped, it's important to document the types of
> errors your functions can throw. Xcode supports a [`Throws`
